...
extern "C" BASICEXTERNALOBJECT_API long moveRect(TaggedData * argv, long argc, TaggedData * retval)
{
	// The returned value type
	retval->type = kTypeScript;

	// Create a script to be run
	string s("var currentComp = app.project.item(2);");
	s.append("var l1 = currentComp.layer('1');");
	s.append("var p = l1('ADBE Transform Group')('ADBE Position').value;");
	s.append("l1('ADBE Transform Group')('ADBE Position').setValue([p[0], p[1] + 1]);");
